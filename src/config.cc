...
const std::map<Optimization, const char*> opt_descriptions = {
    ...
    {O_REMOVE_GROUP,
     {"Remove unnecessary groups: Some parenthesis can be safely removed without changeing the meaning of the grammar. "
      "E.g.: `A (B C) D` becomes `A B C D` or `X (Y)* Z` becomes `X Y* Z`."}},
    {O_SINGLE_CHAR_CLASS,
     {"Convert single character classes to strings: The code generated for strings is simpler than that generated for "
      "character classes. So we can convert for example `[\\n]` to `\"\\n\"`."}},
    //~ {O_CHAR_CLASS_NEGATION, {"Simplify negation of character classes: Negation of character classes can be written
    // as negative character class (e.g. `![\\n]` -> `[^\\n]`)."}},
    {O_DOUBLE_NEGATION,
     {"Removing double negations: Negation of negation can be ignored, because it results in the original term (e.g. "
      "`!(!TERM)` -> `TERM`)."}},
    {O_DOUBLE_QUANTIFICATION,
     {"Removing double quantifications: If a single term is quantified twice, it is always possible to convert this "
      "into a single potfix operator with equel meaning (e.g. `(X+)?` -> `X*`)."}},
    {O_REPEATS,
     {"Removing unnecessary repeats: Joins repeated rules to single quantity. E.g. \"A A*\" -> \"A+\", \"B* B*\" -> "
      "\"B*\" etc."}},
    {O_CONCAT_STRINGS,
     {"String concatenation: Join adjacent string nodes into one. E.g. `\"A\" \"B\"` becomes `\"AB\"`."}},
    {O_CONCAT_CHAR_CLASSES,
     {"Character class concatenation: Join adjacent character classes in alternations into one. E.g. `[AB] / [CD]` "
      "becomes `[ABCD]`."}},
    {O_UNUSED_VARIABLE,
     {"Removing unused variables: Variables denoted in grammar (e.g. `e:expression`) which are not used in any source "
      "oe error block are discarded."}},
    {O_UNUSED_CAPTURE,
