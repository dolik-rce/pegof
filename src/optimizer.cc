...
bool is_in_error_action(Node& node) {
    return node.find_ancestors<Term>(
                   [](const Term& term) -> bool { return term.error_action_contains_any_capture(); }
           ).size()
        > 0;
}
            ...
            if (t.is_simple() && t.contains<String>()) {
                ...
                if (prev_str) {
                    if (is_in_error_action(t)) {
                        log(2,
                            "Not merging adjacent strings (%s + %s), because they might be referenced in error action");
                    } else {
                        log(1,
                            "Merging adjacent strings: \"%s\" + \"%s\"",
                            str.to_c_string().c_str(),
                            prev_str->to_c_string().c_str());
                        str.append(*prev_str);
                        s->erase(prev_term);
                        s->update_parents();
                        optimized++;
            ...
            if (s.has_single_term() && t.contains<CharacterClass>()) {
                if (prev_term && prev_term->same_prefix(t) && prev_term->same_quantifier(t)) {
                    ...
                    if (cc1.is_negative() == cc2.is_negative() && !cc1.any_char() && !cc1.any_char()) {
                        if (is_in_error_action(t)) {
                            log(2,
                                "Not merging character classes (%s + %s), because they might be referenced in error "
                                "action",
                                STR(t),
                                STR(*prev_term));
                        } else {
...
int optimize_repeating_terms(Term& t1, Term& t2) {
    ...
    if (t1.is_greedy() && t2.is_optional()) {
        ...
        case '*': return 1; // delete t1
        case '+': t1.quantifier = 0; return 0;
        case 0:
            t1.quantifier = 0;
            t2.quantifier = '?';
            return 0;
...
int Optimizer::double_negations() {
    ...
    return apply(O_DOUBLE_NEGATION, [](Node& node, int& optimized) -> bool {
        ...
        if (!group.has_single_term()) {
            return false;
        }
        Term inner_term = group.get_first_term();
        if (!inner_term.is_negative()) {
            return false;
        }
...
int Optimizer::double_quantifications() {
    ...
    return apply(O_DOUBLE_QUANTIFICATION, [](Node& node, int& optimized) -> bool {
        ...
        Term inner_term = group.get_first_term();
        if (!inner_term.is_quantified() || inner_term.is_prefixed()) {
            return false;
        }
    ...
    return apply(O_REMOVE_GROUP, [](Node& node, int& optimized) -> bool {
        ...
        if (a) {
            for (int pos = 0; pos < a->size(); pos++) {
                ...
                if (!s.has_single_term()) {
                    continue;
                }
                Term term = s.get_first_term();
                if (!term.is_simple() || !term.contains<Group>()) {
                    continue;
                }
                // A / (B / C) / D -> A / B / C / D
        ...
        Term* t = node.as<Term>();
        if (!t || !t->contains<Group>()) {
            return false;
        }
        Group group = t->get<Group>();
        if (!group.has_single_sequence()) {
            return false;
        }
...
int Optimizer::unused_variables() {
    return apply(O_UNUSED_VARIABLE, [](Node& node, int& optimized) -> bool {
        ...
        std::vector<Action*> actions =
            rule->find_children<Action>([r](const Action& action) -> bool { return action.contains_reference(*r); });
        if (!actions.empty()) {
            return false;
        }
...
int Optimizer::unused_captures() {
    return apply(O_UNUSED_CAPTURE, [](Node& node, int& optimized) -> bool {
        ...
        for (int i = 0; i < captures.size(); i++) {
            ...
            bool used_in_expand =
                std::any_of(expands.begin(), expands.end(), [i](const Expand* expand) { return *expand == i + 1; });
...
int Optimizer::inline_rules() {
    ...
    for (int i = rules.size() - 1; i > 0; i--) {
        ...
        bool is_recursive = !rule.find_children<Reference>(
                                     [rule](const Reference& ref) -> bool { return ref.references(&rule); }
        ).empty();
        if (is_recursive) {
        ...
        std::vector<Reference*> refs =
            g.find_children<Reference>([rule](const Reference& ref) -> bool { return ref.references(&rule); });
        ...
        bool contains_full_rule_ref =
            !rule.find_children<Action>(
                     [rule](const Action& action) -> bool { return action.contains_capture(0); }
            ).empty();
        ...
        bool err_contains_full_rule_ref =
            !rule.find_children<Term>(
                     [rule](const Term& term) -> bool { return term.error_action_contains_capture(0); }
            ).empty();
        ...
        bool has_captures = rule.find_children<Action>(
                                    [](const Action& action) -> bool { return action.contains_any_capture(); }
        ).size();
        if (std::any_of(refs.begin(), refs.end(), [has_captures](Reference* ref) {
                return has_captures
                    && ref->find_ancestors<Term>([](const Term& term) -> bool { return term.is_greedy(); }).size();
            })) {
